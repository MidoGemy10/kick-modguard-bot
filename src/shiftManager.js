...
function addMod({ discordId, kickUsername, kickUserId = null }) {
  const stmt = db.prepare(`
    ...
    ON CONFLICT(discord_id) DO UPDATE SET
      ...
      active = 1
  `);
  stmt.run(String(discordId), kickUserId ? String(kickUserId) : null, String(kickUsername));
...
function getOpenShift(modId) {
  return db.prepare('SELECT * FROM shifts WHERE mod_id = ? AND status = ? ORDER BY id DESC LIMIT 1')
    .get(modId, 'open');
}

async function startShiftForMod(mod) {
  const open = getOpenShift(mod.id);
  if (open) return { alreadyOpen: true, shift: open };
...
async function startShiftByDiscord(discordId) {
  ...
  if (!mod) return { ok: false, reason: 'المود مش متضاف في النظام.' };

  if (config.countOnlyWhenLive && !isStreamLive()) {
    return { ok: false, reason: 'القناة ليست Live الآن، والتسجيل وقت الأوفلاين مقفول.' };
  }

  const result = await startShiftForMod(mod);
  return { ok: true, mod, ...result };
}
...
async function closeShift(shiftId, reason = 'مغلق يدويًا') {
  ...
  db.prepare(`
    UPDATE shifts
    SET ended_at = ?, status = 'closed', close_reason = ?, total_minutes = ?, active_minutes = ?, idle_minutes = ?
    WHERE id = ?
  `).run(endedAt, reason, minutes.total, minutes.active, minutes.idle, shiftId);
  ...
  await logs.logShiftClosed({ mod, shift: closed, reason });
  return closed;
}
...
async function closeShiftByDiscord(discordId, reason = 'خروج يدوي') {
  ...
  if (!shift) return { ok: false, reason: 'لا يوجد شيفت مفتوح لهذا المود.' };
  const closed = await closeShift(shift.id, reason);
  return { ok: true, mod, shift: closed };
}
...
async function updateStreamState(payload) {
  ...
  db.prepare(`
    UPDATE stream_state
    SET is_live = ?, title = ?, started_at = ?, ended_at = ?, last_event_at = ?
    WHERE id = 1
  `).run(isLive, payload.title || null, payload.started_at || null, payload.ended_at || null, nowIso());
...
async function closeAllOpenShifts(reason) {
  const shifts = db.prepare('SELECT * FROM shifts WHERE status = ?').all('open');
...
function markEventProcessed(messageId) {
  ...
  try {
    db.prepare('INSERT INTO processed_events (message_id) VALUES (?)').run(String(messageId));
    return true;
  } catch {
    return false;
  }
...
function getReport(period = 'today') {
  ...
  const now = new Date();
  if (period === 'week') {
    since = new Date(now.getTime() - 7 * 24 * 60 * 60000).toISOString();
  } else if (period === 'month') {
    since = new Date(now.getTime() - 30 * 24 * 60 * 60000).toISOString();
  } else {
